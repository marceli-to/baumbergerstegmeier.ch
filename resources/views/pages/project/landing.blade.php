      ...
      @foreach ($teaser_columns as $items)
        <div>
          @foreach($items as $item)
            ...
            <a 
              href="{{ $is_category ? route('page.project.showByStateAndCategory', ['category' => $category->slug, 'state' => $state->slug, 'project' => $item->project->slug]) : route('page.project.showByState', ['state' => $state->slug, 'project' => $item->project->slug]) }}" 
              title="{{ $item->project->title }}">
              <x-teasers.image :image="$item" />
            </a>
          @endforeach
