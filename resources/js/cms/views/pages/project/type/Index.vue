...
<div>
  ...
  <div v-if="isFetched" class="is-loaded">
    <page-header>
      <h2>Projekt-Typen</h2>
      <router-link :to="{ name: 'project-type-create'}" class="btn-add has-icon">
    ...
    <page-footer>
      <button-back :route="'project-dashboard'">Zurück</button-back>
    </page-footer>
  </div>
</div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      routes: {
        ...
        store: '/api/project/type',
        delete: '/api/project/type',
        order: '/api/project/type/order',
        toggle: '/api/project/type/state',
      },
  ...
  methods: {
    ...
    order() {
      let types = this.data.map(function(type, idx) {
        type.order = idx;
        return type;
      });
      ...
      this.debounce = setTimeout(function() {
        this.debounce = false 
        this.axios.post(`${this.routes.order}`, {types: types}).then((response) => {
