...
<div>
  ...
  <div v-if="isFetched" class="is-loaded">
    <page-header>
      ...
      <router-link :to="{ name: 'project-state-create'}" class="btn-add has-icon">
      ...
      <draggable 
        ...
        <div
          ...
          <list-actions 
            :id="d.id" 
            :record="d"
            :routes="{edit: 'project-state-edit'}"
            @toggle="toggle($event)"
...
export default {
  ...
  data() {
    return {
      ...
      routes: {
        ...
        order: '/api/project/state/order',
        toggle: '/api/project/state/state',
      },
  ...
  methods: {
    ...
    order() {
      let states = this.data.map(function(state, idx) {
        state.order = idx;
        return state;
      });
      ...
      this.debounce = setTimeout(function() {
        this.debounce = false 
        this.axios.post(`${this.routes.order}`, {states: states}).then((response) => {
          this.$notify({type: 'success', text: 'Reihenfolge angepasst'});
        });
      }.bind(this, states), 500);
    },
  }
}
</script>